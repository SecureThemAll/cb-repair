...
class Stats(Task):
    ...
    def __call__(self):
        ...
        for cn in self.challenges:
            challenge = self.kernel.get_challenge(cn)
            stats = challenge.stats()

            if 'hunks' in self.plots:
                _, manifest = challenge.get_manifest(force=True)
                snippets = manifest.get_snippets()
                stats['hunks'] = sum([len(s) for f, s in snippets.items()])
    ...
    def plot(self):
        if self.plots:
            plotter = Plotter(out_path=self.kernel.configs.plots)
            ...
            if 'hunks' in self.plots:
                plotter.histogram([s['hunks'] for s in self.stats.values()], filename='hunks_hist',
                                  title="Histogram of the number of hunks per Challenge",
                                  x_label="Hunks", y_label="Challenges", cmap='RdYlGn')
...
def stats_args(input_parser):
    input_parser.add_argument('--plots', choices=['cwes', 'lines', 'povs', 'hunks'], nargs="+", default=None,
