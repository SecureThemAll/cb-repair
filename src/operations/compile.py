...
class Compile(Make):
    ...
    def _backup_manifest_files(self):
        ...
        with manifest_file.open(mode="r") as mf:
            ...
            for file in files:
                ...
                bckup_file = bckup_path / f"{file_path.stem}_{self.cid}{file_path.suffix}"
                super(Make, self).__call__(cmd_str=f"cp {file} {bckup_file}",
                                           msg=f"Backup of manifest file {file} to {self.backup}.\n",
    ...
    def _build(self):
        ...
        if self.challenge.paths.get_povs():
            cmd_str = f"make {self.challenge.name} pov"

        super(Make, self).__call__(cmd_str=cmd_str, msg=f"Building {self.challenge.name}\n",
    ...
    def __str__(self):
        ...
        if self.fixes:
            compile_cmd_str += f" --fix_files {self.fixes}"

        if self.coverage:
            compile_cmd_str += " --coverage"

        if self.link:
            compile_cmd_str += " --link"

        if self.cpp_files:
            compile_cmd_str += " --cpp_files"

        if self.backup:
            compile_cmd_str += f" --backup {self.backup}"
...
def compile_args(input_parser):
    ...
    input_parser.add_argument('-B', '--backup', type=str, help='Backups the manifest file to a given path.',
                              default=None)
