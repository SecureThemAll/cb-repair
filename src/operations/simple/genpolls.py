...
class GenPolls(SimpleOperation):
    ...
    def __call__(self):
        ...
        if self.kernel.has_sanity(self.challenge.name):
            self.kernel.reset_sanity(self.challenge.name)
            self.kernel.save_metadata()
    ...
    def state_machine(self):
        ...
        pollers = list(self.poller.iterdir())
        ...
        for poll_dir in pollers:
            if poll_dir.is_dir():
                ...
                if state_machine_script.exists() and state_graph.exists():
                    ...
                    cmd_str = f"{self.kernel.configs.python2} -B {self.kernel.get_tools().gen_polls} --count {self.count} " \
